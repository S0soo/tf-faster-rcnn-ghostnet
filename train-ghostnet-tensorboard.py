...
def get_training_roidb(imdb, cantingname, dataname):
    """Returns a roidb (Region of Interest database) for use in training."""
    imdb.cantingname = cantingname
    imdb.dataname = dataname
    if True:
...
def combined_roidb(imdb_names, cantingname, dataname):
    """
    ...
    def get_roidb(imdb_name):
        ...
        roidb = get_training_roidb(imdb, cantingname, dataname)
        return roidb
...
class Train:
    def __init__(self, canting, data_riqi):
        ...
        self.imdb, self.roidb = combined_roidb("voc_2007_trainval", canting, data_riqi)

        self.data_layer = RoIDataLayer(self.roidb, self.imdb.num_classes)
        self.output_dir = cfg.get_output_dir(self.imdb, 'default_{}_{}'.format(self.canting, self.data_riqi))
    ...
    def train(self):
        ...
        while iter < cfg.FLAGS.max_iters + 1:
            ...
            rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, total_loss, summary = self.net.train_step_with_summary(sess,
                                                                                                                   blobs,
                                                                                                                   train_op)
            timer.toc()
            iter += 1
...
if __name__ == '__main__':
    canting = input("输入餐厅名称：")
    data_riqi = input("输入训练日期：")

    train = Train(canting, data_riqi)
    train.train()